...
double interp_2d (double z, double logrho, double *tab) {
    ...
    if (z >= Z_MAX) return 0.0;
    if (logrho < LOGRHO_MIN) return 0.0;
    if (logrho >= LOGRHO_MAX) return 0.0;
    ...
    fy = jp+1+(LOGRHO_MIN-logrho)*LOGRHO_STEPS/(LOGRHO_MAX-LOGRHO_MIN);
...
double ampl (double u, double rho, double *func0, double *func1) {
    ...
    if (u > rho) {
        ...
        if (u <= 10.0*rho) A *= interp_2d(rho/u,log10(rho),func0); 
    } else if (u < rho) {
        A = sqrt(1.0+4.0/rho/rho) * interp_2d(u/rho,log10(rho),func1);
    } else {
        A = (1.0+rho*rho)*(0.5*M_PI+asin((rho*rho-1.0)/(rho*rho+1.0)))/rho/rho;
        A += 2.0/rho;
        A /= M_PI;
    }
    return A;
...
double ampl_ld (double u, double rho, double Gamma, double Lambda, 
    double *func0, double *func1, int nsteps) {
